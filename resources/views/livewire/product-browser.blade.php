                    ...
                    @foreach ($filters as $title => $filter)
                        <div class="space-y-1">
                            ...
                            @foreach ($filter as $option => $count)
                                <div class="flex items-center space-x-2">
                                    <input type="checkbox" wire:model="queryFilters.{{ $title }}" id="{{ strtolower($title) }}_{{ strtolower($option) }}" value="{{ $option }}">
                                    <label for="{{ strtolower($title) }}_{{ strtolower($option) }}">{{ $option }} ({{ $count }})</label>
                                </div>
                            @endforeach
                        </div>
                    @endforeach
